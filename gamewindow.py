...
class GameWindow(QWidget):
    ...
    def run(self):
        ...
        finished = False
        while not finished:
            world[1:-1, 1:-1] = self._world._grid.cells
            if self._world._bc == "Periodic":
                world[1:-1, 0] = self._world._grid.cells[:, -1]
                world[1:-1, -1] = self._world._grid.cells[:, 0]
                world[0, :] = world[-2, :]
                world[-1, :] = world[1, :]
            for i in range(1, ni+1):  # go down each column
                for j in range(1, nj+1):  # go across each row
                    ...
                    self._world._grid.cells[i-1, j-1] = self._world.outcome(
                        world[i, j], num_live_neighbours
                    )
            if not np.any(self._world._grid.cells - world[1:-1, 1:-1]):
                finished = True
            else:
                self._im.set_data(self._world._grid.cells)
                plt.pause(self._world._tick)
